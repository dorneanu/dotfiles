...
(use-package simple
  ...
  :hook ((before-save . delete-trailing-whitespace)
         (mu4e-compose-mode . +text-mode-autofill-setup)
         (markdown-mode . +text-mode-autofill-setup)
         (rst-mode . +text-mode-autofill-setup)
         (git-commit-setup . +text-mode-autofill-setup))
  :config
...
#+end_src
** COMMENT puni - Not using it for now
...
(use-package puni
  ...
  :hook ((prog-mode . puni-mode)
         (go-mode . puni-mode)
         (python-mode . puni-mode))
  :init
  ;; Define a prefix keymap for M-a
  (defvar puni-prefix-map (make-sparse-keymap)
    "Keymap for puni structural editing commands.")
  ;; Bind M-a to the puni keymap
  (global-set-key (kbd "M-a") puni-prefix-map)

  :bind (:map puni-prefix-map
              ;; Basic navigation - use first letters for most common operations
              ("f" . puni-forward-sexp)
              ("b" . puni-backward-sexp)
              ("u" . puni-backward-up-list)
              ("d" . puni-forward-down-list)

              ;; Killing and deletion
              ("k" . puni-kill-line)
              ("w" . puni-kill-word)
              ("K" . puni-delete)  ; Delete expression at point while keeping structure balanced

              ;; Manipulation
              ("s" . puni-splice-sexp)     ; Remove surrounding delimiters
              ("r" . puni-raise-sexp)      ; Replace parent with current sexp
              ("t" . puni-transpose)       ; Swap with next expression

              ;; Selection
              ("SPC" . puni-mark-sexp-at-point)  ; Mark expression
              ("m" . puni-mark-list)             ; Mark containing list

              ;; Wrapping
              ("(" . puni-wrap-round)        ; () parentheses
              ("[" . puni-wrap-square)       ; [] brackets
              ("{" . puni-wrap-curly)        ; {} braces
              ("<" . puni-wrap-angle)        ; <> angle brackets
              ("\"" . puni-wrap-double-quote) ; "" double quotes
              ("'" . puni-wrap-single-quote)  ; '' single quotes
              ("`" . puni-wrap-back-quote)    ; `` back quotes

              ;; Slurping/Barfing - use mnemonic keys
              (">" . puni-slurp-forward)    ; Pull in next expression
              ("<" . puni-barf-forward)     ; Push out last expression
              ("." . puni-slurp-backward)   ; Pull in previous expression
              ("," . puni-barf-backward)    ; Push out first expression

              ;; Force commands for emergency editing
              ("x" . puni-force-delete))    ; Force delete when you need to break structure
...
(use-package awesome-tab
  :straight t
  :init
  ; Define a keymap for our prefix
  (defvar awesome-tab-prefix-map (make-sparse-keymap))
  (global-set-key (kbd "C-c t") awesome-tab-prefix-map)
  :bind (:map awesome-tab-prefix-map
              ;; Basic tab navigation
              ("n" . awesome-tab-forward-tab)
              ("p" . awesome-tab-backward-tab)
              ("f" . awesome-tab-select-beg-tab)
              ("l" . awesome-tab-select-end-tab)
              ("j" . awesome-tab-ace-jump)
              ("o" . awesome-tab-open-tab-in-current-group)

              ;; Group navigation - use another keymap for "g" prefix
              ("g" . nil) ; This makes "g" a prefix key
              ("g n" . awesome-tab-forward-group)
              ("g p" . awesome-tab-backward-group)
              ("g s" . awesome-tab-switch-group)
              ("g i" . awesome-tab-counsel-switch-group)

              ;; Other window navigation
              ("w" . nil) ; This makes "w" a prefix key
              ("w n" . awesome-tab-forward-tab-other-window)
              ("w p" . awesome-tab-backward-tab-other-window)

              ;; Tab movement
              ("m" . nil) ; This makes "m" a prefix key
              ("m r" . awesome-tab-move-current-tab-to-right)
              ("m l" . awesome-tab-move-current-tab-to-left)
              ("m f" . awesome-tab-move-current-tab-to-beg)

              ;; Buffer management
              ("k" . nil) ; This makes "k" a prefix key
              ("k o" . awesome-tab-kill-other-buffers-in-current-group)
              ("k a" . awesome-tab-kill-all-buffers-in-current-group)
              ("k m" . awesome-tab-kill-match-buffers-in-current-group)
              ("k k" . awesome-tab-keep-match-buffers-in-current-group))
  :config
  (setq awesome-tab-ace-str-style 'left)  ;; Can be 'left, 'right, or 'replace-icon
  (setq awesome-tab-ace-keys '(?a ?s ?d ?f ?g ?h ?j ?k ?l))  ;; Keys used for ace jump
  ...
  (defun awesome-tab-hide-tab (x)
    (let ((name (format "%s" x))
          (buffer (get-buffer x)))
      (or
       ;; Original name-based conditions
       (string-prefix-p "*epc" name)
       (string-prefix-p "*helm" name)
       (string-prefix-p "*Compile-Log*" name)
       (string-prefix-p "*lsp" name)
       (string-prefix-p "*LSP" name)
       (string-prefix-p "*kubel" name)
...
#+RESULTS:
: sqrtminusone/rdrview-elfeed-show
...
#+end_src
**** custom functions for gptel
Mostly from https://github.com/gregoryg/emacs-gregoryg/blob/master/README.org
#+begin_src emacs-lisp
...
***** COMMENT Search the web and read a web page
#+begin_src emacs-lisp
...
#+RESULTS:
: #s(gptel-tool gjg/read-web "read_webpage" "Read the content of a URL" ((:name "url" :type "string" :description "URL to read")) nil "web" nil t)
...
#+end_src
*** COMMENT mcp.el - disabled for the moment
#+begin_src emacs-lisp
(use-package mcp
...
(setq mcp-hub-servers
      '(
        ("filesystem" . (:command "npx" :args ("-y" "@modelcontextprotocol/server-filesystem" "$HOME/repos/")))
        ("fetch" . (:command "uvx" :args ("mcp-server-fetch")))
        ))

;; Custom commands for gptel
