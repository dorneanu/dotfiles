...
Disable gnus (not used, causes theme inheritance cycles):
#+begin_src emacs-lisp
;; Prevent standalone gnus from launching, but allow gnus sub-modules
;; to load normally (mu4e depends on gnus-art, gnus-cite, etc.)
(setq gnus-home-directory "~/")

;; Only stub the top-level entry points that would launch gnus itself
(dolist (feature '(ol-gnus nnselect nnir))
  (provide feature))
#+end_src
...
:end:
** navigation
Enhanced keybindings for Super key - organized by function:

#+begin_src emacs-lisp
;; === WINDOW NAVIGATION === (vim-like hjkl)
...
(global-set-key (kbd "s-W") #'delete-other-windows)    ; close other windows
(global-set-key (kbd "s-s") #'split-window-below)      ; split horizontal
(global-set-key (kbd "s-v") #'split-window-right)      ; split vertical
(global-set-key (kbd "s-=") #'balance-windows)         ; balance windows
(global-set-key (kbd "s-f") #'toggle-frame-fullscreen) ; fullscreen

;; === TAB/BUFFER NAVIGATION ===
(global-set-key (kbd "s-.") #'tab-next)
(global-set-key (kbd "s-,") #'tab-previous)
(global-set-key (kbd "s-t") #'tab-new)                 ; new tab
(global-set-key (kbd "s-T") #'tab-close)               ; close tab
(global-set-key (kbd "s-n") #'next-buffer)
(global-set-key (kbd "s-p") #'previous-buffer)

;; === TEXT NAVIGATION === (avoid s-i conflict)
(global-set-key (kbd "s-d") #'forward-paragraph)       ; down paragraph
(global-set-key (kbd "s-u") #'backward-paragraph)      ; up paragraph
(global-set-key (kbd "s-a") #'beginning-of-line)       ; beginning of line
(global-set-key (kbd "s-e") #'end-of-line)             ; end of line

;; === QUICK ACCESS ===
(global-set-key (kbd "s-r") #'consult-recent-file)     ; recent files
(global-set-key (kbd "s-b") #'consult-buffer)          ; switch buffer
(global-set-key (kbd "s-g") #'consult-goto-line)       ; goto line
(global-set-key (kbd "s-/") #'consult-line)            ; search in buffer
(global-set-key (kbd "s-o") #'ace-window)              ; window picker

;; === UTILITY ===
(global-set-key (kbd "s-z") #'undo)                    ; quick undo
(global-set-key (kbd "s-Z") #'undo-redo)               ; quick redo (if available)
(global-set-key (kbd "s-x") #'kill-region)             ; cut
(global-set-key (kbd "s-c") #'kill-ring-save)          ; copy
(global-set-key (kbd "s-y") #'yank)                    ; paste
(global-set-key (kbd "s-SPC") #'set-mark-command)      ; set mark

;; === APPS/MODES ===
(global-set-key (kbd "s-m") #'magit)                   ; quick magit
(global-set-key (kbd "s-;") #'gptel)                   ; quick AI

;; === FILE OPERATIONS ===
(global-set-key (kbd "s-S") #'save-buffer)             ; save file
(global-set-key (kbd "s-O") #'find-file)               ; open file
(global-set-key (kbd "s-N") #'find-file)               ; new file (same as open)
(global-set-key (kbd "s-K") #'kill-buffer)             ; close buffer
(global-set-key (kbd "s-R") #'revert-buffer)           ; reload file

;; === SEARCH & REPLACE ===
(global-set-key (kbd "s-F") #'isearch-forward)         ; find
(global-set-key (kbd "s-H") #'query-replace)           ; find & replace
(global-set-key (kbd "s-*") #'isearch-forward-symbol-at-point) ; find symbol under cursor

;; === TEXT EDITING ===
(global-set-key (kbd "s-L") #'goto-line)               ; go to line number
(global-set-key (kbd "s-D") #'kill-whole-line)         ; delete line
(global-set-key (kbd "s-J") #'join-line)               ; join lines
(global-set-key (kbd "s-i") #'indent-region)           ; indent (now safe!)
(global-set-key (kbd "s-I") #'indent-rigidly)          ; force indent

;; === DEVELOPMENT ===
(global-set-key (kbd "s-`") #'shell)                   ; quick shell
(global-set-key (kbd "s-~") #'eshell)                  ; quick eshell
(global-set-key (kbd "C-c v t") #'vterm)               ; vterm
(global-set-key (kbd "C-c v p") #'projectile-run-vterm) ; projectile vterm
(global-set-key (kbd "C-c v o") #'projectile-run-vterm-other-window) ; projectile vterm other window
(global-set-key (kbd "s-C") #'compile)                 ; compile project
(global-set-key (kbd "s-E") #'eval-buffer)             ; evaluate buffer

;; === ORG MODE HELPERS ===
(global-set-key (kbd "s-[") #'org-agenda)              ; quick agenda
(global-set-key (kbd "s-]") #'org-capture)             ; quick capture
(global-set-key (kbd "s-\\") #'org-cycle-agenda-files) ; cycle org files

;; === DISPLAY & UI ===
(global-set-key (kbd "s-+") #'text-scale-increase)     ; zoom in
(global-set-key (kbd "s--") #'text-scale-decrease)     ; zoom out
(global-set-key (kbd "s-0") #'text-scale-adjust)       ; reset zoom
(global-set-key (kbd "s-TAB") #'other-window)          ; cycle windows

;; === BOOKMARKS & REGISTERS ===
(global-set-key (kbd "s-1") #'bookmark-set)            ; set bookmark
(global-set-key (kbd "s-2") #'bookmark-jump)           ; jump to bookmark
(global-set-key (kbd "s-3") #'point-to-register)       ; save position
(global-set-key (kbd "s-4") #'jump-to-register)        ; jump to position

;; === EMERGENCY & UTILITY ===
(global-set-key (kbd "s-q") #'save-buffers-kill-terminal) ; quit emacs
(global-set-key (kbd "s-!") #'shell-command)           ; run shell command
(global-set-key (kbd "s-&") #'async-shell-command)     ; run async command
#+end_src
...
#+begin_src emacs-lisp
;; Don't foret to handle minibuffer with "minibuffer-keyboard-quit".
...
*** MacOS
#+begin_src emacs-lisp
(setq mac-command-modifier        'meta    ; Left Command = Meta (for M-x)
      mac-right-command-modifier  'super   ; Right Command = Super (for s-k)
...
*** jumps
#+begin_src emacs-lisp
(keymap-global-set "C-c a c" #'avy-goto-char)
(keymap-global-set "C-c a w" #'avy-goto-word-1)
...
(with-eval-after-load 'org
  ...
  (define-key org-mode-map (dorneanu/my-org-key "i i") #'org-download-clipboard)
  ...
  ;; Note: "r" prefix is used for multiple refile commands, so don't bind it directly
  (define-key org-mode-map (dorneanu/my-org-key "k") #'org-store-link)
  (define-key org-mode-map (dorneanu/my-org-key "l") #'org-insert-link)
  (define-key org-mode-map (dorneanu/my-org-key "e m") #'my/toggle-org-emphasis-markers)
  (define-key org-mode-map (dorneanu/my-org-key "e r") #'my/refresh-org-emphasis))
#+end_src
* Completion
** vertico
#+begin_src emacs-lisp
...
Disabled - prefer regular vertico
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(defun my/de-indent ()
  "De-indent current line or region by one tab stop."
  (interactive)
  (if (use-region-p)
...
(use-package go-ts-mode
  ...
  :hook ((go-ts-mode . apheleia-mode)
         (go-ts-mode . electric-pair-local-mode)
         (go-ts-mode . (lambda ()
                         (setq-local indent-tabs-mode t)
                         (setq-local tab-width 4)
                         (setq-local go-ts-mode-indent-offset 4))))
  :bind (:map go-ts-mode-map
              ("<backtab>" . my/de-indent)
              ("C-c g e h" . eldoc-box-hover-mode)
              ("C-c g e e" . my-switch-to-eglot)
              ("C-c g e l" . my-switch-to-lsp)
              ("M-?" . godoc-at-point)
...
;; go-guru removed — lsp-enable-symbol-highlighting covers identifier highlighting
...
(use-package lsp-mode
  ...
  ;; inlay hints
  (lsp-inlay-hint-enable t)                        ; Show parameter names and type hints inline
...
(use-package repeat
  ...
  ;; Allow C-SPC to continue popping mark ring after initial C-u C-SPC
...
(use-package breadcrumb
  :straight t
  :hook ((go-mode go-ts-mode org-mode c-mode c++-mode c-ts-mode c++-ts-mode python-mode python-ts-mode rust-mode rust-ts-mode sh-mode bash-ts-mode) . breadcrumb-local-mode))
#+end_src
*** aggressive-indent
...
(use-package magit
  :straight t
  :init
  (defun claude-code-commit-message ()
    "Generate a commit message from staged diff using claude and copy to clipboard."
    (interactive)
    (let ((diff (shell-command-to-string "git diff --cached")))
      (if (string-empty-p (string-trim diff))
          (message "No staged changes")
        (message "Generating commit message...")
        (let* ((prompt (format "Write ONLY a concise commit message for this diff. No explanation, no markdown fences, just the message. Follow conventional commits style.\n\n```diff\n%s\n```" diff))
               (temp-file (make-temp-file "claude-prompt-" nil ".txt" prompt))
               (buf (generate-new-buffer "*claude-commit-msg*"))
               (process-connection-type nil))
          (set-process-sentinel
           (start-process-shell-command
            "claude-commit-msg" buf
            (format "cat %s | claude -p" (shell-quote-argument temp-file)))
           (lambda (proc _event)
  ...
  (defun my/magit-side-by-side (repo-1 repo-2)
    "Open magit for REPO-1 and REPO-2 side by side."
    (interactive "DFirst repo: \nDSecond repo: ")
    (delete-other-windows)
    (let ((magit-display-buffer-function
           (lambda (buffer) (display-buffer-same-window buffer nil))))
      (magit-status repo-1)
      (split-window-right)
      (other-window 1)
      (magit-status repo-2)
  ...
  :bind (("C-c m m" . magit-status)
         ...
         ("C-c m S" . magit-file-stage)
         ("C-c m M" . my/magit-side-by-side)
         ("C-c m z" . magit-stash)
         :map magit-diff-mode-map
         ("C-c A c" . claude-code-commit-message)
         :map magit-status-mode-map
         ("C-c A c" . claude-code-commit-message)))
#+end_src

#+RESULTS:
: #s(hash-table data (:use-package (27169 29424 784279 0) :init (27169 29424 783230 0) :init-secs (0 0 147 0) :use-package-secs (0 0 8088 0) :config (27169 29424 783219 0) :config-secs (0 0 2 0)))
...
*** forge
#+begin_src emacs-lisp
;; Use latest ghub (has ghub-graphql removed, but we'll patch pr-review)
(use-package ghub
  :straight t
  :config
  (add-to-list 'ghub-github-token-scopes '(repo user)))

(use-package forge
  :straight t
  :after magit
  :custom
  ;; Set required scopes for OAuth
  (forge-github-token-scopes '(repo user))
  :bind (:map forge-pullreq-mode-map
              ("C-c r" . forge-browse-pullreq)
         :map magit-mode-map
              ("C-c f p" . forge-pull))
  :config
  ;; Add company GitHub Enterprise instance
  (push '("github.wdf.sap.corp" "github.wdf.sap.corp/api/v3"
          "github.wdf.sap.corp" forge-github-repository)
        forge-alist)

  ;; Register a custom auth-source backend that delegates to the gh CLI.
  ;; This is cleaner than advising ghub--token directly: any package using
  ;; auth-source (not just ghub/forge) will pick up the token automatically.
  ...
  (cl-defun auth-source-gh-search (&rest spec &allow-other-keys)
    (when-let* ((host (plist-get spec :host))
                ((string-match-p "github\\.wdf\\.sap\\.corp" (format "%s" host))))
      (let* ((token (string-trim (shell-command-to-string
                                  "gh auth token --hostname github.wdf.sap.corp")))
             (user (or (plist-get spec :user) "I748851")))
...
(use-package browse-at-remote
  ...
  :config
  ;; Fix for wrong-type-argument stringp t error
  (defun browse-at-remote--vc-git-call-fix (orig-fun &rest args)
    "Fix vc-git--call being passed t instead of nil as INFILE."
    (let ((infile (car args))
          (rest-args (cdr args)))
      (when (eq infile t)
        (setq infile nil))
      (apply orig-fun infile rest-args)))

  (advice-add 'vc-git--call :around #'browse-at-remote--vc-git-call-fix)
...
*** pr-review
#+begin_src emacs-lisp
;; Use latest pr-review - it has fixes for ghub-graphql deprecation (issue #22)
(use-package pr-review
  :straight t
  :after (magit ghub)
  :config
  (with-eval-after-load 'forge
    (define-key forge-pullreq-mode-map (kbd "C-c r") #'my/pr-review-current-forge-pr))
  ;; Helper function to review current forge PR
  (defun my/pr-review-current-forge-pr ()
    "Start pr-review for the current forge PR buffer."
    (interactive)
    (if-let* ((topic forge-buffer-topic)
              (repo (forge-get-repository topic))
              (owner (oref repo owner))
              (name (oref repo name))
              (number (oref topic number))
              (host (oref repo githost)))
        ;; Set ghub host dynamically: enterprise needs /api/v3 suffix, github.com does not
        (let ((pr-review-ghub-host (if (string= host "github.com")
                                       "api.github.com"
                                     (concat host "/api/v3")))
              (url (format "https://%s/%s/%s/pull/%s" host owner name number)))
          (pr-review url))
      (user-error "Not in a forge PR buffer or PR info not available"))))
#+end_src
...
(use-package org-agenda
  ...
  (setq
   ...
   ;; Dynamic tag column width functions
   org-agenda-tags-column nil  ; Will be set dynamically
   org-tags-column nil         ; Will be set dynamically
   ...
   ;; Remove any nil entries that sneak in (e.g. from missing files/dirs)
   org-agenda-files (delq nil org-agenda-files))
...
(with-eval-after-load 'org
  ...
  ;; Exclude DONE state tasks and archived headings from refile targets
  (defun dorneanu/verify-refile-target ()
    "Exclude todo keywords with a done state and archived headings from refile targets."
...
If prefix ARG, copy instead of move."
  ...
  ;; Set up refile keybindings now that functions are defined
  (define-key org-mode-map (kbd "C-c o r w") #'org-refile)
  (define-key org-mode-map (kbd "C-c o r v") #'+org/refile-to-visible)
  (define-key org-mode-map (kbd "C-c o r f") #'+org/refile-to-current-file)
  (define-key org-mode-map (kbd "C-c o r o") #'+org/refile-to-other-window)
...
(use-package org-modern
  ...
  (org-modern-todo t)
  (org-modern-keyword nil) ;; reduce keyword padding
  (org-modern-priority nil) ;; disable priority styling to reduce conflicts
  (org-modern-todo-faces
   ;; tweak colors, and force it to be monospaced, useful when using `mixed-pitch-mode'.
   '(("TODO" . (:weight normal :foreground "white" :background "#ff6b6b"))
     ("STARTED" . (:weight normal :foreground "white" :background "#ff9500"))
     ("NEXT" . (:weight normal :foreground "white" :background "#007acc"))
     ("WIP" . (:weight normal :foreground "white" :background "#4ecdc4"))
     ("WAITING" . (:weight normal :foreground "black" :background "#ffe66d"))
     ("DONE" . (:weight normal :foreground "white" :background "#51cf66"))
     ("CANCELED" . (:weight normal :foreground "white" :background "#6c757d" :strike-through t))
     ("PROJ" . (:weight normal :foreground "white" :background "#9c27b0"))
     ("MEETING" . (:weight normal :foreground "white" :background "#e91e63"))
     ("REVIEW" . (:weight normal :foreground "white" :background "#ff5722"))
     ("IDEA" . (:weight normal :foreground "black" :background "#ffc107")))))
#+end_src
...
(use-package org-roam
  ...
  ;; (org-roam-completion-everywhere t)  ; DISABLED - causes org-completion-at-point errors
  :config
...
(use-package org-web-tools
  :straight t
  :commands (org-web-tools-insert-link-for-url org-web-tools-read-url-as-org)
  :config
...
#+end_src
** org-contacts
Contact management for mu4e and other mail clients
#+begin_src emacs-lisp
(use-package org-contacts
  :straight t
  :after org
  :custom
  ;; Set the contacts file(s)
  (org-contacts-files (list (expand-file-name "contacts.org.gpg" org-directory)))

  ;; Property names for contact information
  (org-contacts-email-property "EMAIL")
  (org-contacts-tel-property "PHONE")
  (org-contacts-address-property "ADDRESS")
  (org-contacts-birthday-property "BIRTHDAY")

  ;; Birthday format in agenda
  (org-contacts-birthday-format "Birthday: %l (%Y)")

  :config
  ;; Enable completion in message mode (mu4e compose)
  (add-hook 'message-mode-hook
            (lambda ()
              (add-to-list 'completion-at-point-functions
                           'org-contacts-completion-at-point)))

  ;; Enable completion in mu4e compose mode specifically
  ...
  ;; Ensure corfu is enabled in mu4e compose mode
  (with-eval-after-load 'corfu
    (add-hook 'mu4e-compose-mode-hook 'corfu-mode)))

;; Mail aliases from org-contacts
(setq mail-aliases t)
(add-hook 'mu4e-compose-pre-hook 'org-contacts-gnus-store-last-mail)
#+end_src
** presentations
*** org-re-reveal
...
*** Export clock status for macOS display
#+begin_src emacs-lisp
;; Directory for clock state file
(defvar clock-state-dir (expand-file-name "~/.local/share/orgclock/"))
(unless (file-directory-p clock-state-dir)
  (make-directory clock-state-dir t))

(defun my/update-clock-state ()
  "Update clock state file for external display."
  (let ((state-file (expand-file-name "clock_state.txt" clock-state-dir)))
    (if (org-clock-is-active)
        ;; Calculate values in the org buffer context first
        (let* ((clocked-task (with-current-buffer (marker-buffer org-clock-marker)
                             (save-excursion
                               (goto-char org-clock-marker)
                               (org-get-heading t t t t))))
               ;; Calculate current session time
               (session-seconds (floor (org-time-convert-to-integer
                                       (time-subtract (current-time) org-clock-start-time))))
               (session-minutes (/ session-seconds 60))
               (session-hours (/ session-minutes 60))
               (remaining-minutes (% session-minutes 60))
               (current-session-time (if (> session-hours 0)
                                       (format "%d:%02d" session-hours remaining-minutes)
                                       (format "0:%02d" session-minutes)))
               ;; Get total accumulated time for this task (in org buffer context)
               (total-time (with-current-buffer (marker-buffer org-clock-marker)
                           (save-excursion
                             (goto-char org-clock-marker)
                             (org-clock-sum-current-item))))
               (total-hours (/ total-time 60))
               (total-minutes (% total-time 60))
               (total-time-str (format "%d:%02d" total-hours total-minutes)))
          ;; Now write to temp file with calculated values
          (with-temp-file state-file
            (insert (format "SESSION: [%s] TOTAL: [%s]\n%s"
                          current-session-time
                          total-time-str
                          (if (> (length clocked-task) 50)
                              (concat (substring clocked-task 0 47) "...")
                            clocked-task)))))
      ;; Not clocked in - write inactive status
      (with-temp-file state-file
        (insert "Clock: Inactive")))))

;; Timer to update state file periodically while clocked in
(defvar my/clock-timer nil)

(defun my/start-clock-timer ()
  "Start timer to update clock state file."
  (when my/clock-timer
    (cancel-timer my/clock-timer))
  (setq my/clock-timer
        (run-with-timer 1 30 'my/update-clock-state))) ; Update every 30 seconds

(defun my/stop-clock-timer ()
  "Stop clock state file timer and update state."
  (when my/clock-timer
    (cancel-timer my/clock-timer)
    (setq my/clock-timer nil))
  (my/update-clock-state))

;; Hook into clock events
(add-hook 'org-clock-in-hook 'my/start-clock-timer)
(add-hook 'org-clock-out-hook 'my/stop-clock-timer)

;; Update immediately when Emacs starts (in case already clocked in)
(run-with-idle-timer 2 nil 'my/update-clock-state)
#+end_src
...
#+end_src
*** Dynamic Tag Width Functions
#+begin_src emacs-lisp
 ;; Functions to dynamically calculate tag column positions based on window width
 (defun my/org-tags-column-width ()
   "Calculate dynamic tag column width for org-mode.
   Uses 80% of window width, with minimum of 60 and maximum of 120."
   (let* ((win-width (window-width))
          (tag-col (floor (* win-width 0.8))))
     (max 60 (min 120 tag-col))))

 (defun my/org-agenda-tags-column-width ()
   "Calculate dynamic tag column width for org-agenda.
   Uses 75% of window width, with minimum of 50 and maximum of 100."
   (let* ((win-width (window-width))
          (tag-col (floor (* win-width 0.75))))
     (max 50 (min 100 tag-col))))

 (defun my/update-org-tag-columns ()
   "Update org tag columns based on current window width."
   (setq org-tags-column (- (my/org-tags-column-width)))
   (setq org-agenda-tags-column (my/org-agenda-tags-column-width)))

 ;; Update tag columns when window configuration changes
 (add-hook 'window-configuration-change-hook #'my/update-org-tag-columns)
 (add-hook 'window-size-change-functions (lambda (&rest _) (my/update-org-tag-columns)))

 ;; Set initial values
 (my/update-org-tag-columns)
#+end_src

*** Enhanced Org Emphasis Styling
#+begin_src emacs-lisp
 ;; Create custom org-only emphasis faces
 (defface my-org-bold-face
   '((t (:inherit bold :weight ultra-bold :foreground "#e74c3c" :height 1.05)))
   "Custom bold face for org-mode only")

 (defface my-org-italic-face
   '((t (:inherit italic :slant italic :foreground "#3498db" :weight normal)))
   "Custom italic face for org-mode only")

 (defface my-org-underline-face
   '((t (:inherit underline :underline t :foreground "#9b59b6")))
   "Custom underline face for org-mode only")

 ;; Customize org-specific faces only
 (custom-set-faces
  '(org-verbatim ((t (:background "#2c3e50" :foreground "#ecf0f1" :family "monospace"))))
  '(org-code ((t (:background "#34495e" :foreground "#f1c40f" :family "monospace")))))

 ;; Variable to track current emphasis state
 (defvar my/org-emphasis-custom-active nil
   "Whether custom org emphasis faces are currently active.")

 ;; Function to toggle between custom and default emphasis
 (defun my/toggle-org-emphasis ()
   "Toggle between custom and default org emphasis faces."
   (interactive)
   (if my/org-emphasis-custom-active
       ;; Switch to default emphasis
       (progn
         (setq org-emphasis-alist
               '(("*" bold)
                 ...
                 ("~" org-code verbatim)
                 ("+" (:strike-through t))))
         (setq my/org-emphasis-custom-active nil)
         (message "Org emphasis: DEFAULT (standard faces)"))
     ;; Switch to custom emphasis
     (progn
       (setq org-emphasis-alist
             '(("*" my-org-bold-face)
               ("/" my-org-italic-face)
               ("_" my-org-underline-face)
               ("=" org-verbatim verbatim)
               ("~" org-code verbatim)
               ("+" (:strike-through t))))
       (setq my/org-emphasis-custom-active t)
       (message "Org emphasis: CUSTOM (colored faces)")))
 ...
 ;; Start with default emphasis (no custom faces applied by default)
 (with-eval-after-load 'org
   (setq my/org-emphasis-custom-active nil)
   (setq org-emphasis-alist
         '(("*" bold)
           ...
           ("~" org-code verbatim)
           ("+" (:strike-through t))))
   ;; Keybinding for toggle function
   (define-key org-mode-map (kbd "C-c o e t") 'my/toggle-org-emphasis))

 ;; Function to toggle emphasis markers visibility
 (defun my/toggle-org-emphasis-markers ()
 ...
 ;; Helper function to refresh org emphasis styling
 (defun my/refresh-org-emphasis ()
   "Refresh org emphasis styling in current buffer."
   (interactive)
   (when (derived-mode-p 'org-mode)
     (org-restart-font-lock)
     (message "Org emphasis styling refreshed")))
#+end_src
*** Week headings 
**** Jump to current week heading
 #+begin_src emacs-lisp
 (defun my/org-jump-to-week ()
   "Jump to current ISO week heading using consult-outline with week pre-filled."
   (interactive)
   (let ((week (format-time-string "%V")))
     (minibuffer-with-setup-hook
         (lambda () (insert (format "wk%s" week)))
       (consult-outline))))

 (with-eval-after-load 'org
   (define-key org-mode-map (kbd "C-c g w") #'my/org-jump-to-week))
 #+end_src
**** Create week headings
 #+begin_src emacs-lisp
 (defun my/org-create-week-heading-work ()
   "Create or jump to current ISO week heading in work org file."
   (interactive)
   (let* ((week (format-time-string "%V"))
          (year (format-time-string "%Y"))
          (week-heading (format "* wk%s - %s" week year))
          (work-file (expand-file-name "work.org" org-directory)))
     (find-file work-file)
     (goto-char (point-min))
     (if (search-forward week-heading nil t)
         (progn
           (org-show-subtree)
           (message "Jumped to existing week heading: wk%s" week))
       (goto-char (point-max))
       (unless (bolp) (insert "\n"))
       (insert week-heading "\n")
       (org-show-subtree)
       (message "Created new week heading: wk%s" week))))

 (defun my/org-create-week-heading-private ()
   "Create or jump to current ISO week heading in private org file."
   (interactive)
   (let* ((week (format-time-string "%V"))
          (year (format-time-string "%Y"))
          (week-heading (format "* wk%s - %s" week year))
          (private-file (expand-file-name "private.org" org-directory)))
     (find-file private-file)
     (goto-char (point-min))
     (if (search-forward week-heading nil t)
         (progn
           (org-show-subtree)
 ...
 (with-eval-after-load 'org
   (define-key org-mode-map (kbd "C-c g W") #'my/org-create-week-heading-work)
   (define-key org-mode-map (kbd "C-c g P") #'my/org-create-week-heading-private))
 #+end_src
* Media
** empv
...
(use-package elfeed
  ...
  :bind (;; Global bindings
         ...
         ("m e" . my/elfeed-export-entries)
         ("m E" . my/elfeed-export-clear)
         ("m s" . my/elfeed-export-summarize)
         ("m d" . my/elfeed-export-dired)
         ...
         ("y" . elfeed-show-yank)
         ("e" . elfeed/elfeed-show-visit-eww)
         ("n" . elfeed-show-next)
         ("p" . elfeed-show-prev)
         ("+" . elfeed-show-tag)
         ("-" . elfeed-show-untag)
  ...
  :config
  ;; Enable images in elfeed entries
  ...
  ;; Show which feed is being fetched during elfeed-update
  (defvar my/elfeed-update-count 0)
  (defvar my/elfeed-update-total 0)

  (defun my/elfeed-update-start (&rest _)
    (setq my/elfeed-update-count 0
          my/elfeed-update-total (length elfeed-feeds))
    (message "elfeed: updating %d feeds..." my/elfeed-update-total))

  (defun my/elfeed-update-feed-notify (url)
    (cl-incf my/elfeed-update-count)
    (message "elfeed [%d/%d]: %s" my/elfeed-update-count my/elfeed-update-total url))

  (advice-add 'elfeed-update :before #'my/elfeed-update-start)
  (advice-add 'elfeed-update-feed :before #'my/elfeed-update-feed-notify)
...
;; ── Feed-grouped sort ──────────────────────────────────────────────────────
;; Sort entries by feed title (alphabetically), then by date descending within
;; each feed group.  This replaces the default date-only sort.
(defun dorneanu/elfeed-sort-by-feed-then-date (a b)
  "Sort elfeed entries: group by feed title (A-Z), then newest-first within group."
  (let* ((feed-a (elfeed-entry-feed a))
         (feed-b (elfeed-entry-feed b))
         (title-a (or (and feed-a (or (elfeed-meta feed-a :title)
                                      (elfeed-feed-title feed-a)))
                      ""))
         (title-b (or (and feed-b (or (elfeed-meta feed-b :title)
                                      (elfeed-feed-title feed-b)))
                      "")))
    (if (string= title-a title-b)
        ;; Same feed → sort newest-first by date
        (> (elfeed-entry-date a) (elfeed-entry-date b))
      ;; Different feeds → sort A-Z by feed title
      (string< title-a title-b))))

;; ── Grouped entry rendering ───────────────────────────────────────────────
;; elfeed maps entries to lines as (offset + index) — any extra inserted line
;; breaks that and corrupts single-line redraws.  So we never insert extra lines.
;; Instead, we show the feed name on EVERY line but dim it for all entries after
;; the first in a group, so it reads as a visual group without extra rows.
;;
;; We still need to know "is this the first entry in its feed group?" so we
;; track the previous entry's feed via the elfeed-search-entries list.

(defun dorneanu/elfeed--prev-feed-title (entry)
  "Return the feed title of the entry immediately before ENTRY in the search list, or nil."
  (let* ((entries elfeed-search-entries)
         (idx (cl-position entry entries :test #'eq)))
    (when (and idx (> idx 0))
      (let* ((prev (nth (1- idx) entries))
             (prev-feed (elfeed-entry-feed prev)))
        (when prev-feed
          (or (elfeed-meta prev-feed :title)
              (elfeed-feed-title prev-feed)))))))

(defun dorneanu/elfeed--next-feed-title (entry)
  "Return the feed title of the entry immediately after ENTRY in the search list, or nil."
  (let* ((entries elfeed-search-entries)
         (idx (cl-position entry entries :test #'eq)))
    (when (and idx (< (1+ idx) (length entries)))
      (let* ((next (nth (1+ idx) entries))
             (next-feed (elfeed-entry-feed next)))
        (when next-feed
          (or (elfeed-meta next-feed :title)
              (elfeed-feed-title next-feed)))))))

(defun dorneanu/elfeed-entry-line-draw-grouped (entry)
  "Draw ENTRY with feed shown prominently for group-first entries, dimmed otherwise."
  (let* ((feed (elfeed-entry-feed entry))
         (feed-title (or (and feed (or (elfeed-meta feed :title)
                                       (elfeed-feed-title feed)))
                         ""))
         (prev-feed-title (dorneanu/elfeed--prev-feed-title entry))
         (next-feed-title (dorneanu/elfeed--next-feed-title entry))
         (is-first-in-group (not (equal feed-title prev-feed-title)))
         (is-last-in-group  (not (equal feed-title next-feed-title)))
         ...
         ;; Age
         (age-in-days (/ (- (float-time) (float-time (elfeed-entry-date entry))) 86400))
         (age-face (cond ((> age-in-days 365) 'font-lock-comment-face)
                         ((> age-in-days 30)  'font-lock-keyword-face)
                         (t                   'font-lock-string-face)))

         ;; Fixed column widths — independent of elfeed-goodies settings
         ;; date(16) + sp(1) + feed(22) + sp(1) + tags(22) + sp(1) = 63 overhead
         (feed-col-width 22)
         (tag-col-width  22)
         (title-width    (max 20 (- (window-width) feed-col-width tag-col-width 19)))

         ;; Formatted columns — all truncated to their fixed width
         (feed-column (elfeed-format-column feed-title feed-col-width :left))
         (tag-column  (elfeed-format-column tags-str   tag-col-width  :left))

         ;; Feed face: bold+bright for first entry in group, dimmed for rest
         (feed-face (if is-first-in-group
                        '(:inherit elfeed-search-feed-face :weight bold)
                      'font-lock-comment-face)))

    (if (>= (window-width) (* (frame-width) elfeed-goodies/wide-threshold))
        (progn
          (insert (propertize date        'face 'elfeed-search-date-face) " ")
          (insert (propertize feed-column 'face feed-face) " ")
...
(defun dorneanu/elfeed-apply-group-spacing (&rest _)
  "After a full redraw, add line-spacing to the newline at each group boundary."
  (when (buffer-live-p (elfeed-search-buffer))
    (with-current-buffer (elfeed-search-buffer)
      (let ((inhibit-read-only t))
        (save-excursion
          (cl-loop for i from 0 below (1- (length elfeed-search-entries))
                   do (let* ((entry (nth i elfeed-search-entries))
                             (next  (nth (1+ i) elfeed-search-entries))
                             (feed  (elfeed-entry-feed entry))
                             (nfeed (elfeed-entry-feed next))
                             (ft    (when feed  (or (elfeed-meta feed  :title) (elfeed-feed-title feed)  "")))
                             (nft   (when nfeed (or (elfeed-meta nfeed :title) (elfeed-feed-title nfeed) ""))))
                        (when (and ft nft (not (equal ft nft)))
                          ;; Go to line i+offset, find its trailing newline, set line-spacing
                          (elfeed-goto-line (+ elfeed-search--offset i))
                          (end-of-line)
                          (when (eq (char-after) ?\n)
                            (put-text-property (point) (1+ (point)) 'line-spacing 0.8))))))))))

(advice-add 'elfeed-search-update :after #'dorneanu/elfeed-apply-group-spacing)
#+end_src
...
(use-package elfeed-goodies
  ...
  :config
  ;; Use grouped draw: feed-group headers + newest-first within each group
  (setq elfeed-search-print-entry-function #'dorneanu/elfeed-entry-line-draw-grouped)
  (setq elfeed-search-sort-function #'dorneanu/elfeed-sort-by-feed-then-date))
#+end_src
*** Youtube
...
(defun my/eww-to-epub ()
  "Convert current eww buffer to epub.
Uses the raw HTML source already fetched by eww (:source), falling
back to curl if :source is nil. No org-web-tools/plz involved."
  (interactive)
  (unless (derived-mode-p 'eww-mode)
    (user-error "Current buffer is not an eww buffer"))
  (let* ((url (eww-current-url))
         (title (or (plist-get eww-data :title) url "Untitled"))
         (source (plist-get eww-data :source))
         (clean-filename (my/sanitize-filename title))
         (dated-filename (format "%s_%s"
                                 (format-time-string "%Y-%m-%d")
                                 clean-filename))
         (temp-html (make-temp-file "eww-export" nil ".html"))
         (epub-file (expand-file-name
                     (concat dated-filename ".epub")
                     "~/Dropbox/Ebooks/")))
    (if source
        (progn
          (message "Exporting '%s' from eww source..." title)
          (let ((coding-system-for-write 'utf-8))
            (with-temp-file temp-html (insert source))))
      (message "No cached source, fetching '%s' via curl..." url)
      (call-process "curl" nil nil nil
                    "-sL" "--max-time" "30" "--user-agent" "Mozilla/5.0"
                    url "-o" temp-html))
    (make-directory (file-name-directory epub-file) t)
    (call-process "pandoc" nil nil nil
                  "-f" "html" "-t" "epub3"
                  "--metadata" (format "title=%s" title)
                  "-o" epub-file temp-html)
    (delete-file temp-html)
    (let ((dropbox-dest (format "db:Apps/Dropbox Pocketbook/articles/%s"
                                (format-time-string "%Y"))))
      (message "Created epub: %s — uploading to Dropbox..." (expand-file-name epub-file))
      (start-process "rclone-upload" nil "rclone" "copy"
                     epub-file dropbox-dest)
      (message "Created epub: %s — uploaded to %s" (expand-file-name epub-file) dropbox-dest))))
...
** elfeed export for LLM analysis
Export elfeed entries as org files for offline LLM analysis.
For Reddit threads, uses =reddigg= to render post + comments into org.
For HN threads, uses =hnreader= to render post + comments into org.
For other URLs, uses =org-web-tools= for readable content.

#+begin_src emacs-lisp
(defvar my/elfeed-export-dir "~/tmp/elfeed-export/"
  "Directory where exported elfeed entries are saved.")

(defun my/elfeed-export-clear ()
  "Delete all exported files in `my/elfeed-export-dir'."
  (interactive)
  (let ((dir (expand-file-name my/elfeed-export-dir)))
    (if (and (file-directory-p dir)
             (yes-or-no-p (format "Delete everything in %s? " dir)))
        (progn
          (delete-directory dir t)
          (make-directory dir t)
          (message "Cleared %s" dir))
      (message "Aborted."))))

(defun my/elfeed-export-summarize ()
  "Send exported elfeed org files to claude-code for summarization."
  (interactive)
  (require 'claude-code)
  (let* ((dir (expand-file-name my/elfeed-export-dir))
         (files (directory-files-recursively dir "\\.org$"))
         (prompt (if files
                     (format "Read all the org files in %s and give me a summary of the key topics, discussions, and takeaways across all articles. Group related topics together." dir)
                   nil)))
    (if prompt
        (progn
          (claude-code--do-send-command prompt)
          (message "Sent %d files to claude-code for summarization" (length files)))
      (user-error "No org files found in %s" dir))))

(defun my/elfeed-export-dired ()
  "Open the elfeed export directory in dired."
  (interactive)
  (let ((dir (expand-file-name my/elfeed-export-dir)))
    (if (file-directory-p dir)
        (dired dir)
      (user-error "Export directory %s does not exist" dir))))

(defun my/elfeed-export--reddit-p (url)
  "Return non-nil if URL is a Reddit URL."
  (string-match-p "reddit\\.com" url))

(defun my/elfeed-export--hn-p (entry)
  "Return non-nil if ENTRY is from Hacker News."
  (let ((tags (elfeed-entry-tags entry))
        (url (elfeed-entry-link entry)))
    (or (memq 'hn tags)
        (string-match-p "news\\.ycombinator\\.com" url)
        (string-match-p "hacker-news" (or (elfeed-feed-title (elfeed-entry-feed entry)) "")))))

(defun my/elfeed-export--hn-comments-url (entry)
  "Extract HN comments URL from ENTRY content."
...
(defun my/elfeed-export--sanitize-filename (name)
  "Sanitize NAME for use as a filename."
  (let ((clean (replace-regexp-in-string "[^a-zA-Z0-9_-]" "-" name)))
    (replace-regexp-in-string "-+" "-"
      (replace-regexp-in-string "^-\\|-$" "" clean))))

(defun my/elfeed-export--reddit-to-file (url title filename callback)
  "Fetch Reddit thread at URL via reddigg, save org content to FILENAME.
Calls CALLBACK with FILENAME when done (async)."
  (require 'reddigg)
  (let ((cmt-path (if (string-prefix-p "https" url)
                      (substring url (length "https://old.reddit.com/"))
                    url)))
    ;; Remove trailing slashes and query params for clean path
    (when (string-prefix-p "https://www.reddit.com/" url)
      (setq cmt-path (substring url (length "https://www.reddit.com/"))))
    (promise-chain (reddigg--promise-comments cmt-path)
      (then (lambda (data)
              ;; Render into a temp buffer (same as reddigg--print-comments but we capture it)
              (let ((buf (generate-new-buffer " *elfeed-export-reddit*")))
                (with-current-buffer buf
                  (insert "#+startup: overview indent\n")
                  (insert (format "#+title: %s\n" title))
                  (insert (format "#+date: %s\n" (format-time-string "%Y-%m-%d %H:%M")))
                  (insert (format "#+property: URL %s\n\n" url))
                  ;; Print post content
                  (let ((cmt (ht-get* (aref (ht-get* (aref data 0) "data" "children") 0) "data"))
                        (op-url (ht-get (ht-get* (aref (ht-get* (aref data 0) "data" "children") 0) "data") "url"))
                        begin end)
                    ;; Add OP link if it's a link post (not self post)
                    (when (and op-url (not (string-prefix-p "https://www.reddit.com/r/" op-url)))
                      (insert (format "#+property: OP_LINK %s\n\n" op-url)))
                    (insert "* Post\n")
                  ...
                  ;; Print comments
                  (insert "\n* Comments\n")
                  (reddigg--print-comment-list
                   (ht-get* (aref data 1) "data" "children") "*")
                  ;; Save to file
                  (write-region (point-min) (point-max) filename)
                  (kill-buffer buf))
                (message "Exported Reddit: %s → %s" title (file-name-nondirectory filename))
                (when callback (funcall callback filename)))))
      (promise-catch (lambda (err)
                       (message "Failed to export Reddit '%s': %s" title err)
                       (when callback (funcall callback nil)))))))

(defun my/elfeed-export--hn-to-file (comments-url title filename callback)
  "Fetch HN thread at COMMENTS-URL via hnreader, save org content to FILENAME.
Calls CALLBACK with FILENAME when done (async)."
  (require 'hnreader)
  (promise-chain (hnreader--promise-dom comments-url)
    (then (lambda (dom)
            (let ((buf (generate-new-buffer " *elfeed-export-hn*"))
                  (comments (dom-by-class dom "^athing comtr$"))
                  (hn-title (hnreader--get-title dom))
              ...
              (with-current-buffer buf
                ...
                (insert (format "#+date: %s\n" (format-time-string "%Y-%m-%d %H:%M")))
                (insert (format "#+property: URL %s\n" comments-url))
                ;; OP's article link from the HN post
                (when (cdr hn-title)
                  (insert (format "#+property: OP_LINK %s\n" (cdr hn-title))))
                (insert "\n")
                ;; Post info
                ...
                (dolist (comment comments)
                  ...
                  (hnreader--print-node
                   (dom-append-child (hnreader--get-comment comment) '(p))))
                ;; Save to file
                (write-region (point-min) (point-max) filename)
                (kill-buffer buf))
              (message "Exported HN: %s → %s" title (file-name-nondirectory filename))
              (when callback (funcall callback filename)))))
    (promise-catch (lambda (err)
                     (message "Failed to export HN '%s': %s" title err)
                     (when callback (funcall callback nil))))))

(defun my/elfeed-export--article-to-file (url title filename)
  "Fetch article at URL using org-web-tools, save to FILENAME (sync)."
  (require 'org-web-tools)
  (let ((org-content (org-web-tools--url-as-readable-org url)))
    (with-temp-file filename
      ...
      (if org-content
          (insert org-content)
        (insert "Failed to fetch content.")))
    (message "Exported: %s → %s" title (file-name-nondirectory filename))))

(defun my/elfeed-export-entries ()
  "Export marked (or current) elfeed-search entries as org files.
For Reddit URLs, uses reddigg to fetch post + comments (async).
For HN entries, uses hnreader to fetch post + comments (async).
For other URLs, uses org-web-tools for readable content (sync).
Saves to `my/elfeed-export-dir'."
  (interactive)
  (let* ((entries (elfeed-search-selected))
         (date-dir (format-time-string "%Y-%m-%d"))
         (export-dir (expand-file-name date-dir my/elfeed-export-dir))
    ...
    (dolist (entry entries)
      (let* ((title (elfeed-entry-title entry))
             (url (elfeed-entry-link entry))
             (slug (my/elfeed-export--sanitize-filename
                    (truncate-string-to-width (or title "untitled") 80)))
             (filename (expand-file-name (concat slug ".org") export-dir)))
        (message "Exporting [%d/%d]: %s" (1+ (- total (length entries))) total title)
        (cond
         ;; Reddit entries
         ((my/elfeed-export--reddit-p url)
          (cl-incf async-pending)
          (my/elfeed-export--reddit-to-file url title filename
            (lambda (_) (cl-decf async-pending)
              (when (zerop async-pending)
                (message "All async exports done. Files in: %s" export-dir)))))
         ;; HN entries
         ((my/elfeed-export--hn-p entry)
          (let ((comments-url (my/elfeed-export--hn-comments-url entry)))
            (if comments-url
                (progn
                  (cl-incf async-pending)
                  (my/elfeed-export--hn-to-file comments-url title filename
                    (lambda (_) (cl-decf async-pending)
                      (when (zerop async-pending)
                        (message "All async exports done. Files in: %s" export-dir)))))
              (message "No HN comments URL found for '%s', exporting as article" title)
              (condition-case err
                  (progn
                    (my/elfeed-export--article-to-file url title filename)
                    (cl-incf sync-exported))
                (error (message "Failed to export '%s': %s" title (error-message-string err)))))))
         ;; Regular articles
         (t
          (condition-case err
              (progn
    ...
    (message "Exported %d sync entries. %d async (Reddit/HN) pending. Dir: %s"
             sync-exported async-pending export-dir)
    (dired export-dir)))

(defun my/reddit-urls-from-region-or-buffer ()
  "Return list of Reddit URLs from the active region, or the whole buffer."
    ...
    (let (urls)
      (with-temp-buffer
        ...
        (while (re-search-forward
                "https?://\\(?:www\\|old\\)\\.reddit\\.com/r/[^\s\n\r\"'<>]+" nil t)
          (push (match-string 0) urls)))
      (nreverse urls))))

(defun my/export-reddit-urls-to-org ()
  "Export Reddit URLs from the active region (or whole buffer) to org files.
Each URL is fetched via reddigg and saved to `my/elfeed-export-dir'.
Works from any buffer (e.g. *scratch*, notes)."
  (interactive)
  (let* ((urls (my/reddit-urls-from-region-or-buffer))
         (date-dir (format-time-string "%Y-%m-%d"))
         (export-dir (expand-file-name date-dir my/elfeed-export-dir))
         (total (length urls))
         (pending total))
    (unless urls
      (user-error "No Reddit URLs found in %s"
                  (if (use-region-p) "region" "buffer")))
    (make-directory export-dir t)
    (dolist (url urls)
      (let* ((slug (if (string-match "/comments/[^/]+/\\([^/?]+\\)" url)
                       (match-string 1 url)
                     url))
             (slug (my/elfeed-export--sanitize-filename
                    (truncate-string-to-width slug 80)))
             (filename (expand-file-name (concat slug ".org") export-dir)))
        (message "Fetching [%d]: %s" total url)
        (my/elfeed-export--reddit-to-file
         url slug filename
         (lambda (result)
           (cl-decf pending)
           (if result
               (message "Saved: %s" (file-name-nondirectory filename))
             (message "Failed: %s" url))
           (when (zerop pending)
             (message "Done. %d Reddit threads exported to: %s" total export-dir)
             (dired export-dir))))))
    (message "Fetching %d Reddit thread(s) (async)..." total)))
#+end_src
...
;; activate org-modern-mode
(add-hook 'hnreader-mode-hook #'org-modern-mode)
#+end_src
...
(defun my/elfeed-hn-show-comments (&optional link)
  ...
  (let* ((entry (if (eq major-mode 'elfeed-show-mode)
    ...
    (run-with-timer 0.5 nil
                    (lambda ()
                      (when (string-match "\\*HNComments\\*" (buffer-name))
                        (org-modern-mode 1))))
    ))
#+end_src
...
(defun my/dired-play-wav ()
  "Play WAV file at point with afplay (macOS)."
  (interactive)
  (let ((file (dired-get-file-for-visit)))
    (if (string-match-p "\\.wav\\'" file)
        (progn
          (start-process "afplay" nil "afplay" file)
          (message "Playing WAV: %s" (file-name-nondirectory file)))
      (message "Not a WAV file"))))

(defun my/kill-afplay ()
  "Kill all running afplay processes."
  (interactive)
  (let ((killed 0))
    (dolist (proc (process-list))
      (when (string-match-p "afplay" (process-name proc))
        (delete-process proc)
        (setq killed (1+ killed))))
    (message "Killed %d afplay process%s" killed (if (= killed 1) "" "es"))))

(with-eval-after-load 'dired
  (define-key dired-mode-map (kbd "C-c w") #'my/dired-play-wav)
  (define-key dired-mode-map (kbd "C-c K") #'my/kill-afplay))
#+end_src
** recentf
#+begin_src emacs-lisp
(use-package recentf
...
#+begin_src emacs-lisp
  ...
  Most importantly, your outputs should reflect the same quality, tone, and brand voice as my previous successful blog posts. Your role is to elevate the post's structure and writing, not to drastically alter its character. Please let me know if you need any clarification or have additional suggestions to improve this system prompt."))
    ...
    (gptel-make-openai "OpenWebUI"
      :protocol "http"
      :host "localhost:3000"  ; 3000 with your OpenWebUI host/port
      :endpoint "/api/chat/completions"
      :stream t
      :key "sk-db1bc3d539f14d6eb84bab26fd83b2f1"
      :models '("sap_ai_core.gemini-2.5-pro" "sap_ai_core.anthropic--claude-4.5-sonnet" "sap_ai_core.anthropic--claude-4-sonnet")
      :request-params '(:parallel_tool_calls t))

    ;; OpenRouter (OpenAI-compatible, supports many models including Nvidia Nemotron)
    (gptel-make-openai "OpenRouter"
      :host "openrouter.ai"
      :endpoint "/api/v1/chat/completions"
      :key (password-store-get "emacs/openrouter")
      :stream t
...
#+end_src
*** COMMENT ellama
Not used anymore. claude-code is now the primary choice. 
#+begin_src emacs-lisp
(use-package ellama
  :straight t
  :defer t
  :commands (ellama-transient-main-menu ellama-chat ellama-ask-about ellama-complete)
...
(use-package claude-code
  :straight (:type git :host github :repo "manzaltu/claude-code.el" :branch "main" :depth 1
  ...
  ;; Display Claude Code buffer on the right side by default
  (add-to-list 'display-buffer-alist
               '("\\*claude-code\\*"
...
;;   :straight (:type git :host github :repo "manzaltu/claude-code-ide.el" :branch "main" :depth 1)
...
;; Claude Code endpoint and account switching functions
(defvar claude-code-current-profile "personal"
  "Current Claude Code profile (personal or work).")

;; By default, use personal account (settings.json)
;; Switch to work account when needed (settings.work.json)

(defun claude-code-use-work-account ()
  "Switch Claude Code to use work account settings."
  (interactive)
  (setq claude-code-program-switches (list "--settings" (expand-file-name "~/.claude/settings.work.json")))
...
(defun claude-code-use-personal-account ()
  ...
  (interactive)
  (setq claude-code-program-switches nil)  ; Use default settings.json
  (setq claude-code-current-profile "personal")
  (message "Claude Code: Switched to personal account (default). Start a new Claude session."))
...
(defun claude-code-restart-with-account (account)
  "Restart Claude Code with the specified ACCOUNT (personal or work)."
  (interactive
   (list (completing-read "Select account: " '("personal" "work") nil t)))
  (if (string= account "work")
      (progn
        (setq claude-code-program-switches (list "--settings" (expand-file-name "~/.claude/settings.work.json")))
        (setq claude-code-current-profile "work")
        (message "Claude Code: Switching to work account..."))
    (progn
      (setq claude-code-program-switches nil)
      (setq claude-code-current-profile "personal")
      (message "Claude Code: Switching to personal account...")))
...
(setq mu4e-bookmarks
      `(
        ...
        ("from:hello@readwise.io AND NOT flag:trashed" "Readwise" ?r)
        ("from:peter@golangweekly.com AND NOT flag:trashed" "Golang Weekly" ?G)
        ("from:stareanatiei@substack.com AND NOT flag:trashed" "Starea Nației" ?S)
        ;; (,(s-join " "
...
(use-package password-store
  :straight t
  :commands (password-store-copy password-store-get password-store-insert))
(use-package pass
  :straight t
  :commands (pass))
#+end_src
...
(use-package avy
  ...
  :bind (("C-c a c" . avy-goto-char)
         ...
         ("C-c a z" . avy-zap-to-char-dwim)
         ("C-c a k" . avy-kill-ring-save-region)
         ("C-c a K" . avy-kill-region)
         ("C-c a y" . avy-kill-ring-save-whole-line)
...
(use-package isearch
  ...
  (isearch-regexp-lax-whitespace t) ; spaces match anything on the line in regexp isearch
...
** UI Polish
Subtle visual improvements for better aesthetics and readability.
...
#+begin_src emacs-lisp
  ...
  (setq show-paren-delay 0.1
        show-paren-style 'mixed) ; highlight paren and expression when offscreen

  ;; Subtle window dividers
...
*** COMMENT Ligatures
Disabled due to org-mode tangling issues with << and >> characters.
#+begin_src emacs-lisp
;; Enable programming ligatures for better visual flow
...
(use-package emojify
  ...
  ;; TODO keyword colors moved to org-mode section
...
(use-package windmove
  ...
  (keymap-global-set "C-c w [" #'shrink-window-horizontally)
  (keymap-global-set "C-c w ]" #'enlarge-window-horizontally)
  (keymap-global-set "C-c w -" #'shrink-window)
  (keymap-global-set "C-c w =" #'enlarge-window)

  ;; Make window resize commands repeatable
  (dolist (cmd '(shrink-window-horizontally
                 enlarge-window-horizontally
                 shrink-window
                 enlarge-window))
    (put cmd 'repeat-map '+windmove-keys)))
#+end_src
*** winner-mode
#+begin_src emacs-lisp
(use-package winner
  ...
  ;; Make C-x 1 reversible - toggle between single window and previous layout
...
Automatically copy text when I drag-select with the mouse:
