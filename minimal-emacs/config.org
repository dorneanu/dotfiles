...
#+begin_src emacs-lisp
;; Prevent gnus from loading - not used and causes face inheritance issues with themes
;; Provide all gnus modules and stub out required functions
(setq gnus-home-directory nil)

;; Stub out gnus functions that might be called
(defun gnus-image-type-available-p (type) nil)
(defun gnus-continuum-version (&optional version) "0.0.0")

;; Provide all gnus-related features to prevent loading
(dolist (feature '(gnus gnus-group gnus-sum gnus-spec gnus-util gnus-range
                   gnus-int gnus-undo gnus-art nnheader nnselect nnir
                   ol-gnus gnus-msg gnus-cite gnus-async))
  (provide feature))
#+end_src
