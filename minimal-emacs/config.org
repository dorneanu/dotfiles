...
(use-package forge
  ...
  ;; Add advice to ghub to use gh CLI token
...
(use-package pr-review
  ...
  (setq pr-review-github-host "github.wdf.sap.corp")

  ;; Add enterprise GitHub to ghub hosts
  (with-eval-after-load 'ghub
    (add-to-list 'ghub-github-token-scopes '(repo user))

    ;; Ensure ghub uses the right API host for enterprise GitHub
    (defun my/pr-review-ghub-host-fix (orig-fun &rest args)
      "Fix ghub to use enterprise GitHub API host."
      (let ((url (car args)))
        (when (and url (stringp url)
                   (string-match-p "github\\.wdf\\.sap\\.corp" url))
          ;; Replace api.github.com with enterprise API endpoint
          (setcar args (replace-regexp-in-string
                       "api\\.github\\.com"
                       "github.wdf.sap.corp/api/v3"
                       url)))
        (apply orig-fun args)))

    (advice-add 'ghub-request :around #'my/pr-review-ghub-host-fix)))
#+end_src
