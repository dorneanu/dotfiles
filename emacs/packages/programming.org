...
*** apheleia-mode
Like black (for Python) or Prettier (for JS related):
...
(defun golang/set-company-backends ()
 (setq-local company-backends '((company-go company-dabbrev-code company-capf company-yasnippet))))
...
(add-hook 'terraform-mode-hook (lambda ()
   (setq-local company-backends '((company-terraform :with company-dabbrev-code company-capf))))
)
